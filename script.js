...
class P{constructor(){this.reset()}reset(){this.x=Math.random()*W;this.y=Math.random()*H;this.size=Math.random()*1.5+.5;this.sx=(Math.random()-.5)*.3;this.sy=(Math.random()-.5)*.3;this.op=Math.random()*.3+.1;this.life=0;this.max=Math.random()*300+200}update(){this.x+=this.sx;this.y+=this.sy;this.life++;if(this.life>this.max||this.x<0||this.x>W||this.y<0||this.y>H)this.reset()}draw(){ctx.save();ctx.globalAlpha=this.op*(1-this.life/this.max);ctx.fillStyle='#E48F24';ctx.beginPath();ctx.arc(this.x,this.y,this.size,0,Math.PI*2);ctx.fill();ctx.restore()}}
for(let i=0;i<80;i++)particles.push(new P());
function anim(){ctx.clearRect(0,0,W,H);for(let i=0;i<particles.length;i++){for(let j=i+1;j<particles.length;j++){const dx=particles[i].x-particles[j].x,dy=particles[i].y-particles[j].y,d=Math.sqrt(dx*dx+dy*dy);if(d<90){ctx.save();ctx.globalAlpha=.1*(1-d/90);ctx.strokeStyle='#E48F24';ctx.lineWidth=.5;ctx.beginPath();ctx.moveTo(particles[i].x,particles[i].y);ctx.lineTo(particles[j].x,particles[j].y);ctx.stroke();ctx.restore()}}particles[i].update();particles[i].draw()}requestAnimationFrame(anim)}
anim();
...
const TARIFAS={'50000':{p:50,l:'Centro Metepec',c:'#6BAF82'},'50001':{p:50,l:'Metepec Centro',c:'#6BAF82'},'50002':{p:80,l:'Metepec Norte',c:'#E48F24'},'50003':{p:80,l:'Metepec Sur',c:'#E48F24'},'50004':{p:80,l:'Metepec Oriente',c:'#E48F24'},'50005':{p:120,l:'Zona lÃ­mite Metepec',c:'#E05555'},'50010':{p:50,l:'San JerÃ³nimo Chicahualco',c:'#6BAF82'},'50020':{p:80,l:'La AsunciÃ³n',c:'#E48F24'},'50040':{p:80,l:'San BartolomÃ© Tlaltelulco',c:'#E48F24'},'50090':{p:80,l:'San Francisco Coaxusco',c:'#E48F24'},'50110':{p:80,l:'San Mateo Atenco',c:'#E48F24'},'50140':{p:80,l:'San Lorenzo Tlalmimilolpan',c:'#E48F24'},'50160':{p:80,l:'Mexicaltzingo',c:'#E48F24'},'50170':{p:80,l:'Calimaya',c:'#E48F24'},'50200':{p:120,l:'Zinacantepec',c:'#E05555'},'52140':{p:80,l:'Toluca Centro',c:'#E48F24'},'52000':{p:80,l:'Toluca',c:'#E48F24'}};
function calcularCP(){
  ...
  let t=TARIFAS[cp];
  if(!t){if(cp.startsWith('500'))t={p:80,l:'Zona Metepec estimada',c:'#E48F24'};else if(cp.startsWith('521')||cp.startsWith('520'))t={p:80,l:'Toluca zona estimada',c:'#E48F24'};}
  r.className='calc-result show';
  if(t){r.style.cssText='display:flex;justify-content:space-between;align-items:center;margin-top:14px;padding:14px;border-radius:10px;background:rgba(0,0,0,0.3);border:1px solid '+t.c+'44';r.innerHTML='<div><div style="font-size:11px;color:#7F7B70;margin-bottom:4px">'+t.l+'</div><div style="font-size:12px;color:rgba(241,226,200,0.75)">Costo estimado de recolecciÃ³n</div></div><span style="font-family:\'Playfair Display\',serif;font-size:26px;font-weight:700;color:'+t.c+'">$'+t.p+' <span style="font-size:13px">MXN</span></span>';}
